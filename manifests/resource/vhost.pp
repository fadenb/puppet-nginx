...
define nginx::resource::vhost (
  ...
  if (! (is_hash($location_cfg_prepend) or is_array($location_cfg_prepend) ) ){
    fail('location_cfg_prepend is not an array nor a hash!')
  }
  if (! (is_hash($location_cfg_append) or is_array($location_cfg_append) ) ){
    fail('location_cfg_append is not an array nor a hash!')
  }
  if ($try_files != undef) {
